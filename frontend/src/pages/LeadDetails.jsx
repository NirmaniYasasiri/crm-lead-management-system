...
function LeadDetails() {
  ...
  // Convert status into CSS class name
  const getStatusClass = (statusText) => {
    return statusText.toLowerCase().replace(/\s+/g, "-");
  };

  // Load one lead with its notes
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      ...
      // Reload lead details to show new note
      loadLead();
    } catch (error) {
    ...
    <>
      ...
      <main className="container">
        ...
        <div className="details-card">
          ...
          <p>
            <strong>Assigned Salesperson:</strong> {lead.assignedSalesperson}
          </p>
          <p>
            <strong>Status:</strong>{" "}
            <span className={`status-badge status-${getStatusClass(lead.status)}`}>
              {lead.status}
            </span>
          </p>
          <p>
            <strong>Estimated Deal Value:</strong> Rs. {lead.estimatedDealValue}
          </p>
          <p>
